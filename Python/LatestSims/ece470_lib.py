...
#             print('poo')
        ...
        for j in range(theta.size):
            ...
            while(theta[j]<-np.pi):
                theta[j]+=2*np.pi
        if(np.linalg.norm(V) <= max_err and isValidTheta(theta,ang)):
            theta_cop = theta
            novel = True
            for a in outMat:
...
#                 print(tcur-tprev)
                ...
                thetaMat,norm = findIK(goalT, ang, S,M,max_iter=100);
                clear_output(wait=True)
                if(len(thetaMat)>0):
                    print('found sokution',len(thetaMat))
    #                 print(thetaMat,"\n")
                    valCount+=1
...
#                     thetaMap[i][j][k]=thetaMat;
                    ...
                    invalCount+=1
                tcur = time()
                print((tcur-tstart)/totCount*(X.size*Y.size*Z.size-totCount)/60,'min until done')
                tprev = tcur
